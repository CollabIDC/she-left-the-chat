...
const Stories = () => {
  ...
  const showFilters = stream === "view-from-here";
  const showJournalTagline = stream === "she-actually-did-it";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        {/* Stream selector */}
        <section className="bg-background pt-4 pb-10">
          <div className="container mx-auto px-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 max-w-3xl mx-auto">
              <button
                onClick={() => setStream("view-from-here")}
                className={cn(
                  "group flex items-center justify-center gap-3 px-6 py-5 rounded-2xl border-2 transition-all",
                  stream === "view-from-here"
                    ? "bg-gold border-gold text-charcoal"
                    : "bg-surface border-charcoal/15 text-charcoal hover:border-gold",
                )}
              >
              ...
              <button
                ...
                className={cn(
                  "px-6 py-5 rounded-2xl border-2 font-label text-xs uppercase tracking-[0.18em] transition-all",
                  stream === "all"
                    ? "bg-charcoal border-charcoal text-ivory"
                    : "bg-surface border-charcoal/15 text-charcoal/70 hover:border-charcoal",
                )}
              >
                Show All
              </button>
              ...
              <button
                onClick={() => setStream("she-actually-did-it")}
                className={cn(
                  "group flex items-center justify-center gap-3 px-6 py-5 rounded-2xl border-2 transition-all",
                  stream === "she-actually-did-it"
                    ? "bg-terracotta border-terracotta text-ivory"
                    : "bg-surface border-charcoal/15 text-charcoal hover:border-terracotta",
                )}
              >
                <BookOpen className="w-5 h-5" />
                <span className="font-display text-base sm:text-lg font-semibold italic">
                  She Actually Did It
                </span>
              </button>
            </div>
        ...
        {/* Filter rows — only for View From Here */}
        {showFilters && (
        ...
        {/* Journal tagline — only for She Actually Did It */}
        {showJournalTagline && (
          <section className="bg-background pb-12">
            <div className="container mx-auto px-6">
              <p className="font-body italic text-sm text-charcoal/70 text-center max-w-2xl mx-auto">
                The unfiltered truth about choosing yourself after 50. Real days.
                Real feelings. No filter.
              </p>
            </div>
          </section>
        )}
